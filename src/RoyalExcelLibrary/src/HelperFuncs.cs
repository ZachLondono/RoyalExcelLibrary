...
namespace RoyalExcelLibrary.ExcelUI {
	...
	public static class HelperFuncs {
        ...
        public static Excel.Worksheet LoadTemplate(string path, string worksheetname, Excel.Workbook workbook) {
            try {
                Excel.Worksheet outputsheet = workbook.Worksheets[worksheetname];
                outputsheet.Delete();
            } catch (COMException) {
                Debug.WriteLine("Output sheet could be deleted or does not exist");
            }
            Excel.Application app = (Excel.Application)ExcelDnaUtil.Application;
            Excel.Workbook template = app.Workbooks.Open(path);
